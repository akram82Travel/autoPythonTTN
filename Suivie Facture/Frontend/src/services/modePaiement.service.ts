...
export class modePaiementService {
  private apiUrl = 'http://127.0.0.1:8000/ModePaiement';
  ...
  async getmodePaiementFact(Fact_num: string): Promise<any> {
    const body = { FCT_num: Fact_num };
    try {
      const response = await this.http.post<any>(this.apiUrl2, body).toPromise();
